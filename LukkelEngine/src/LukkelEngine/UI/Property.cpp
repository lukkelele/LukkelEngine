...
namespace LukkelEngine {
	...
	namespace UI {
		...
		void Property::vec3Control(const std::string& label, glm::vec3& values, float resetValue, float columnWidth)
		{
		...
		void Property::vec4Control(const std::string& label, glm::vec4& values, float min, float max, float resetValue, float columnWidth)
		{
			ImGuiIO& io = ImGui::GetIO();
			auto boldFont = io.Fonts->Fonts[0];
			float speed = 0.01f;
		...
		void Property::colorMenu(Material& material, float columnWidth)
		{
			ImGuiIO& io = ImGui::GetIO();
			auto boldFont = io.Fonts->Fonts[0];
			float lineHeight = GImGui->Font->FontSize + GImGui->Style.FramePadding.y * 2.0f;
